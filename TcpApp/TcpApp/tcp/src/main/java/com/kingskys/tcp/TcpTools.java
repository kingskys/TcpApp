...
public class TcpTools {
    ...
    // 将int转换成4个字节的byte数组
    public static byte[] int2bytes(int v, boolean isBigEndian) {
        byte[] buf = new byte[4];
        if (isBigEndian) {
    ...
    // byte数组转成int
    public static int bytes2int(byte[] src, int pos, boolean isBigEndian) {
        if (pos > 0) {
            byte[] buf = new byte[4];
    ...
    public static byte[] joinBytes(byte[] data1, int pos1, int len1, byte[] data2, int pos2, int len2) {
    ...
    public static byte[] joinBytes(byte data1, byte[] data2, int pos2, int len2) {
        int len1 = 1;
        int length = len1 + len2;
        byte[] newData = new byte[length];
        newData[0] = data1;
    ...
    public static byte[] joinBytes(byte[] data1, int pos1, int len1, byte data2) {
        int len2 = 1;
        int length = len1 + len2;
        byte[] newData = new byte[length];
        System.arraycopy(data1, pos1, newData, 0, len1);
